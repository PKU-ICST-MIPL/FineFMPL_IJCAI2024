    ...
    def forward(self, x_ori, clip_weights_ori, keys=None, obj_branch=0, classnames=None, session_flag=0, eval_flag=0, map_keys=None, map_net=None):
        ...
        if obj_branch == -1:
            ...
            if self.dataset_name == 'cub200':
                ...
                if eval_flag==0:
                    if session_flag == 0:
                        ctx = self.ctx[:100, :, :] + text_prompt_residual.unsqueeze(1)
                    else:
                        ctx = self.ctx[100+(session_flag-1)*10 : 100+(session_flag-1)*10+10, :, :] + text_prompt_residual.unsqueeze(1)
                else:
                ...
                if eval_flag==0:
                    if session_flag == 0:
                        ctx = self.ctx[:60, :, :] + text_prompt_residual.unsqueeze(1)
                    else:
                        ctx = self.ctx[60+(session_flag-1)*5 : 60+(session_flag-1)*5+5, :, :] + text_prompt_residual.unsqueeze(1)
                else:
    ...
    def forward(self, x_ori, clip_weights_ori, keys=None, obj_branch=0, classnames=None, session_flag=0, eval_flag=0, map_keys=None, map_net=None):
        ...
        if obj_branch == -1:
            ...
            if self.dataset_name == 'cub200':
                ...
                if eval_flag==0:
                    if session_flag == 0:
                        ctx = self.ctx[:100, :, :] + text_prompt_residual
                    else:
                        ctx = self.ctx[100+(session_flag-1)*10 : 100+(session_flag-1)*10+10, :, :] + text_prompt_residual
                else:
                ...
                if eval_flag==0:
                    if session_flag == 0:
                        ctx = self.ctx[:60, :, :] + text_prompt_residual
                    else:
                        ctx = self.ctx[60+(session_flag-1)*5 : 60+(session_flag-1)*5+5, :, :] + text_prompt_residual
                else:
    ...
    for train_idx in range(cfg['train_epoch']):
        ...
        if 1:
            ...
            if flag == 0:
                ...
                test_classnames = temp_classnames
                ...
                test_classnames = total_classnames + temp_classnames
            ...
            te_keys_save = te_keys.clone()
            te_keys_obj_save = te_keys_obj[:,0,:].clone()



            #print(te_keys_save.shape)
            #print(te_keys_obj_save.shape)



            te_keys_save = te_keys_save / te_keys_save.norm(dim=-1, keepdim=True)
            te_keys_obj_save = te_keys_obj_save / te_keys_obj_save.norm(dim=-1, keepdim=True)
            ...
            affinity, image_features_new, clip_weights_new, glo_logits = adapter(test_features, te_clip_weigts, keys = te_keys, classnames = test_classnames, session_flag=session_flag, eval_flag=1, 
            ...
            #torch.save(image_features_new, "./fg-cmp_glo_feature.pt")
            #torch.save(test_labels, "./fg-cmp_glo_label.pt")



            torch.save(te_keys_save, "./glo_visual_prototype.pt")
            torch.save(te_keys_obj_save, "./obj_visual_prototype.pt")

            torch.save(clip_weights_new, "./glo_text_prototype.pt")



            affinity, image_features_new, clip_weights_new, obj_logits = adapter_obj(test_features_obj, te_clip_weigts, keys = te_keys_obj, classnames = test_classnames, obj_branch=1, session_flag=session_flag, eval_flag=1,
                                                                         map_keys=map_keys, map_net=adapter.meta_net)
            cache_logits = ((-1) * (beta - beta * affinity)).exp() @ te_values
            cache_logits = alpha * cache_logits
            ...
            aff = []
            for i in range(12):
                sim = 100. * image_features_new[:,i+1,:] @ clip_weights_new
                aff.append(sim)

            aff = torch.stack(aff, dim=0)
            clip_logits = torch.mean(aff, dim=0) + 100. * image_features_new[:,0,:] @ clip_weights_new
            '''
            ...
            torch.save(clip_weights_new, "./obj_text_prototype.pt")




            tip_logits = tip_logits1 + obj_open*factor*tip_logits2
            ...
            print("\n**** test image number: {:.1f}. ****\n".format(test_labels.shape[0]))
            print("\n**** our test accuracy: {:.2f}. ****\n".format(acc))
