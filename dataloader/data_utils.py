...
def get_dataloader(args,session):
    if session == 0:
        ...
        trainset, trainloader, testloader, trainloader_cache = get_new_dataloader(args, session)
    return trainset, trainloader, testloader, trainloader_cache

def get_base_dataloader(args):
    txt_path = "/home/sunhongbo/DATA/FSCIL_data/index_list/" + args.dataset + "/session_" + str(0 + 1) + '.txt'
    ...
    trainloader_cache = torch.utils.data.DataLoader(dataset=trainset, batch_size=args.batch_size_base, shuffle=False,
                                              num_workers=8, pin_memory=True)
...
def get_new_dataloader(args,session):
    txt_path = "/home/sunhongbo/DATA/FSCIL_data/index_list/" + args.dataset + "/session_" + str(session + 1) + '.txt'
    if args.dataset == 'cifar100':
    ...
    if args.batch_size_new == 0:
        ...
        trainloader_cache = torch.utils.data.DataLoader(dataset=trainset, batch_size=batch_size_new, shuffle=False,
                                                  num_workers=8, pin_memory=True)
        ...
        trainloader_cache = torch.utils.data.DataLoader(dataset=trainset, batch_size=args.batch_size_new, shuffle=False,
                                                  num_workers=8, pin_memory=True)
